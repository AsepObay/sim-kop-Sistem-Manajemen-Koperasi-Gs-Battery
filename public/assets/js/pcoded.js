...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (sidebar_hide) {
    sidebar_hide.addEventListener('click', function () {
      ...
      sidebar_hide.setAttribute('aria-expanded', String(
        window.innerWidth <= 1024
          ? sidebar.classList.contains('mob-sidebar-active')
          : !sidebar.classList.contains('pc-sidebar-hide')
      ));
    });
  }
  ...
  // Profile dropdown: simple toggle behavior (no overlay). Add click/outside handlers.
  if (document.querySelectorAll('.user-profile').length) {
    document.querySelectorAll('.user-profile').forEach(function (wrap) {
      var toggle = wrap.querySelector('.profile-toggle');
      ...
      toggle.addEventListener('click', function (e) {
        e.stopPropagation();
        if (dropdown.getAttribute('aria-hidden') === 'false') closeDrop(); else openDrop();
      });
